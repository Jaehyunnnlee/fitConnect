...
//    }
    ...
    public ResponseEntity<?> readComments(@PathVariable("id") Long postNum,Authentication authentication){
            List<CommentResponseDto> commentResponseDtoList=commentService.readComments(postNum,authentication);
    ...
    @PutMapping("/rest-api/comments/{commentNum}")
    public ResponseEntity<?> updateComment(@PathVariable("commentNum") Long commentNum,CommentRequestDto requestDto,Authentication authentication){
        try{
            CommentResponseDto comment=commentService.updateComment(commentNum,requestDto,authentication);
            return ResponseEntity.ok(comment);
        }catch(IllegalArgumentException e){
            return ResponseEntity.badRequest().body(e.getMessage());
        }
